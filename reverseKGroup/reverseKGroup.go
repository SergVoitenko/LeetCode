...
func reverseKGroup(head *ListNode, k int) *ListNode {
	...
	var startPoint, penultimatePoint, penultimatePointOld, endPoint, returnList, nextPoint *ListNode
	i := 1
	first := true
	for head != nil {
		...
		if i == k {
			...
			if first {
				...
				penultimatePoint.Next = nextPoint
				returnList = endPoint
				penultimatePointOld = penultimatePoint
				first = false
			} else {
				endPoint.Next = startPoint
				penultimatePoint.Next = nextPoint
				penultimatePointOld.Next = endPoint
				penultimatePointOld = penultimatePoint
			}
			head = nextPoint
			i = 1
